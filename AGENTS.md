# Working agreement — rsbuild-plugin-ui-css-purge
...
**JavaScript/TypeScript library** (`@pathscale/rsbuild-plugin-ui-css-purge`), built with `bun`.
...
**Always paste the full PR URL** (`https://github.com/pathscale/rsbuild-plugin-ui-css-purge/pull/<n>`), not just the number, so it's
clickable.
...
uncomment the rule below.
    ...
    ```bash
    gh pr view <number> --repo pathscale/rsbuild-plugin-ui-css-purge --json statusCheckRollup
    ```
